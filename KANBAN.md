...
| 항목 | 우선순위 | 담당 | 비고 |
|------|---------|------|------|
| **이론·가설 검증 (Leo 지시 7/10)** | **Top** | reklcli | 1차 판정 완료 (reports/theory/VERDICT_2026-07-10.md): 신강약 87.5% ✅ / 용신 미달 → 원인 기반 규칙로 개선 후 재판정 / 격국 자평진전 75% ✅ |
| 용신 규칙 v2 (원인 기반) | High | reklcli | 태왕→식상, 인성과다→재성, 비겁왕→관성. 재측정으로 60%/80% 기준 재도전 |
| 오염 차단 UX 결정 | High | **Leo** | 퀴즈가 선천 배너를 응답 전 표시 → 자기귀인 오염. "설문 완료 후 노출"로 변경 필요 (EVIDENCE_AUDIT 참조) |
