...
| 항목 | 우선순위 | 담당 | 비고 |
|------|---------|------|------|
| 게이트 수정안 v1.1 승인 | High | **Leo** | 노출 전 원칙 + nc 플래그 + 신봉도 층화 (ENGINE_V02_DESIGN §4). 구현 완료, 발효만 승인 필요 |
...
| 날짜 | 항목 |
|------|------|
| 2026-07-12 | **오염 완화책 3·4 완료**: 어휘 분리(사전+페르소나 리라이트+가드 테스트), nc/신봉도 메타 문항, 하네스 v1.1(노출 전 서브셋·CONTAMINATION_FLAG·신봉도 층화). §4 수정안 등록 — **발효는 Leo 승인 대기**. 버그픽스 2건(클라이언트 지장간 본기, innate 집계 공회전) |
