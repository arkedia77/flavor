...
- **발신 속도 규율 4조** (admin 08-09 19:03 P0 발효): `to` 수신처 **5곳 이상**이면 발신 전 「사전 반증 1회」.
  ...
  ⚠**미결 확인 1건**: 이 규율이 `CHANNEL_RULES.md`·킷 어디에도 **미착지**(19:05 실측 — 문서류 3건은
  session log·fableself notes·sync script 주석이라 규범 정본 아님). 발효 96초 뒤라 admin의 다음 단계일 수
  있어 지적 보류. **다음 수신함 점검 때 재확인** → 여전히 미착지면 CHANNEL_RULES 조항 상정(소유=admin,
  상정은 누구나). 착지 안 되면 브로드캐스트는 어느 세션 컨텍스트에도 안 남아 다음 세션이 규율을 모른다.
