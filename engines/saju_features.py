...
SCHEMA_VERSION = "sf-4"  # sf-4: 방합/삼합 국(局) 감지 → 별격 감지 세력 조정 (2026-07-12)
...
DEFAULT_PARAMS = {
    ...
    # 국(局) 감지 v1 (sf-4) — 국 참여 지지의 지장간을 국 오행으로 화(化)하는 비율.
    # 별격 감지 전용 (기본 피처·신강약에는 미적용). VERDICT_2026-07-12_guk.md
    "guk_full_blend": 1.0,   # 완전 삼합/방합 (3글자)
    "guk_half_blend": 0.5,   # 반합 (왕지 포함 2글자)
    # 전왕: 관성 천간 투출(합거 제외) 시 파격 — 고전 교리. "guk"=국 조정 경로만
    # (국 화(化)로 부풀린 세력에 대한 가드), "all"=항상, "off"=미적용
...
# ── 국(局) 상수 (sf-4) ──
# 삼합: (지지 3글자, 화(化) 오행, 왕지 — 반합 성립에 필수인 가운데 글자)
_SAMHAP = [("신자진", "수", "자"), ("해묘미", "목", "묘"),
           ("인오술", "화", "오"), ("사유축", "금", "유")]
# 방합(계절 국): 반합 불인정 — 삼합과 달리 기세 결속이 아니라 동기(同氣) 회합
_BANGHAP = [("인묘진", "목"), ("사오미", "화"), ("신유술", "금"), ("해자축", "수")]
# 국 오행 대표 천간 (양간) — 그룹/오행 단위 세력 계산엔 극성 무관
_EL_YANG_STEM = {"목": "갑", "화": "병", "토": "무", "금": "경", "수": "임"}
...
def _detect_guk(chart) -> list:
    """지지의 삼합/방합/반합(왕지 포함) 국 감지 (sf-4).

    반환: [{"type": "삼합"|"방합"|"반합", "element": 오행, "branches": [글자...]}]
    같은 오행이라도 삼합·방합이 각각 성립하면 둘 다 반환 (조정 시 글자별 max blend).
    """
    chars = {b for _tag, b, _h in chart["branches"]}
    guks = []
    for trio, el, wang in _SAMHAP:
        inter = set(trio) & chars
        if len(inter) == 3:
            guks.append({"type": "삼합", "element": el, "branches": sorted(inter)})
        elif len(inter) == 2 and wang in inter:
            guks.append({"type": "반합", "element": el, "branches": sorted(inter)})
    for trio, el in _BANGHAP:
        inter = set(trio) & chars
        if len(inter) == 3:
            guks.append({"type": "방합", "element": el, "branches": sorted(inter)})
    return guks


def _guk_adjusted_forces(chart, guks, params) -> dict:
    """국 참여 지지의 지장간을 국 오행으로 화(化)한 조정 세력 (sf-4).

    별격 감지 전용 — 기본 피처(sipsin/strength/elements)는 건드리지 않는다.
    회국(會局) 시 지지가 본래 지장간 대신 국 오행으로 작용한다는 교리를
    blend 비율(완전 국 1.0, 반합 0.5)로 반영.
    반환: {"score", "groups", "weighted", "득령", "득지"} — _special_gyeokguk의
    판정 입력과 동일한 의미 공간.
    """
    ...
    # 글자별 최강 국 적용 (삼합+방합 중복 참여 시 blend 큰 쪽)
    blend_of = {}
    for g in guks:
        blend = (params["guk_full_blend"] if g["type"] in ("삼합", "방합")
                 else params["guk_half_blend"])
        for ch in g["branches"]:
            if ch not in blend_of or blend > blend_of[ch][1]:
                blend_of[ch] = (g["element"], blend)

    # 조정 슬롯 전개 (_weighted_slots와 동일 구조, 국 참여 지지만 치환)
    pw, hw = params["palace_weights"], params["hidden_weights_by_len"]
    slots = [(stem, pw[tag], tag) for tag, stem in chart["stems"]]
    for tag, branch, hidden in chart["branches"]:
        hit = blend_of.get(branch)
        blend = hit[1] if hit else 0.0
        if hit:
            slots.append((_EL_YANG_STEM[hit[0]], pw[tag] * blend, tag))
        if blend < 1.0:
            for stem, w in zip(hidden, hw[len(hidden)]):
                slots.append((stem, pw[tag] * w * (1.0 - blend), tag))
    ...
    total = sum(w for _, w, _ in slots)
    score = sum(w * support(STEM_ELEMENT[s]) for s, w, _ in slots) / total

    groups = {g: 0.0 for g in SIKSHIN_GROUPS}
    for s, w, _ in slots:
        groups[_GROUP_OF[_sipsin_of(day_stem, s)]] += w / total

    weighted = {el: 0.0 for el in ELEMENTS}
    weighted[day_el] += 1.0
    for s, w, _ in slots:
        weighted[STEM_ELEMENT[s]] += w
    wt = sum(weighted.values())
    weighted = {el: v / wt for el, v in weighted.items()}

    # 득령: 월지가 완전 국에 화했으면 국 오행 기준, 아니면 본기 기준
    m_branch, m_hidden = next((b, h) for tag, b, h in chart["branches"] if tag == "월지")
    m_hit = blend_of.get(m_branch)
    m_el = m_hit[0] if (m_hit and m_hit[1] >= 1.0) else STEM_ELEMENT[m_hidden[0]]
    deukryeong = 1.0 if m_el == rel["비겁"] else (0.75 if m_el == rel["인성"] else 0.0)

    # 득지: 완전 화한 지지는 국 오행으로만 통근, 부분 화는 원 지장간도 유효
    root_w, root_total = 0.0, 0.0
    for tag, b, hidden in chart["branches"]:
        w = 1.5 if tag == "일지" else 1.0
        root_total += w
        hit = blend_of.get(b)
        if hit and hit[1] >= 1.0:
            rooted = hit[0] == day_el
        else:
            rooted = any(STEM_ELEMENT[s] == day_el for s in hidden) or (
                hit is not None and hit[0] == day_el)
        if rooted:
            root_w += w
    deukji = root_w / root_total

    return {"score": score, "groups": groups, "weighted": weighted,
            "득령": deukryeong, "득지": deukji}
...
def _special_gyeokguk(chart, sipsin_groups, strength, elements, p):
    ...
    # 국(局) 감지 시 조정 세력으로 판정 (sf-4) — 회국하면 지지는 국 오행으로 작용
    guks = _detect_guk(chart)
    if guks:
        adj = _guk_adjusted_forces(chart, guks, p)
        groups, score, weighted = adj["groups"], adj["score"], adj["weighted"]
        deukji = adj["득지"]
        deukryeong_ok = adj["득령"] > 0
        guk_tags = [f"{g['type']}{g['element']}" for g in guks]
    else:
        ...
        deukji = strength["득지"]
        m_el = STEM_ELEMENT[month_hidden[0]]
        deukryeong_ok = m_el in (rel["비겁"], rel["인성"])
        guk_tags = []
    ...
    for (a, b), hua in _HAP_PAIRS.items():
        ...
        if (partner in adjacent
            ...
            return _mark({"name": name, "group": "별격", "subtype": "합화",
    ...
    if (len(present) == 2
        ...
        if other:
            ...
            if grp in _YANGGI_GUK:
                return _mark({"name": f"양기성상격 {_YANGGI_GUK[grp]}", "group": "별격",
                              "subtype": "양기성상", "tugan": False,
                              "sunse_element": other})

    # 3) 전왕격(일행득기/종강): 극신강 + 관살 무력 + 득령
    #    관성 천간 투출 시 파격 (고전 교리) — 단 인접 천간합으로 합거되면 예외
    mode = p["special_jw_gwan_tugan_block"]
    gwan_tugan = False
    if mode == "all" or (mode == "guk" and guk_tags):
        order = ["년간", "월간", "일간", "시간"]
        stem_at = dict(chart["stems"])
        stem_at["일간"] = day_stem
        seq = [stem_at[t] for t in order if t in stem_at]

        def _is_hap(x, y):
            return (x, y) in _HAP_PAIRS or (y, x) in _HAP_PAIRS

        for i, s in enumerate(seq):
            if _GROUP_OF[_sipsin_of(day_stem, s)] != "관성":
                continue
            hapgeo = any(_is_hap(s, seq[j])
                         for j in (i - 1, i + 1) if 0 <= j < len(seq))
            if not hapgeo:
                gwan_tugan = True
                break

    if (score >= p["special_jw_score"] and not gwan_tugan
            and groups["관성"] <= p["special_jw_gwan_max"] and deukryeong_ok):
        sub = "종강격" if groups["인성"] > groups["비겁"] else _JEONWANG_NAME[day_el]
        return _mark({"name": f"전왕격 {sub}", "group": "별격", "subtype": "전왕",
