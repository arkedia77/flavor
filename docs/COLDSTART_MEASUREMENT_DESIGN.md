...
- **온보딩 문항(프론트 배선 완료 2026-07-16)**: 마지막 문항 후 1문항 —
  "커피는 보통 뭐로 드세요? (한 줄, 예: 아메리카노 진하게 / 바닐라라떼)". 자유 입력 →
  `seeds:["..."]`로 submit에 전달. 별도 테이블 불필요.
  - **게이트**: `/api/coldstart-config`(읽기 전용, `{seed_collection}`) → quiz-engine.js가
    플래그 ON일 때만 seed 화면 동적 주입(HTML 쉘 무변경). OFF=문항 미노출=현 흐름 항등.
  - 커버리지: shared 엔진 vol4~20. 레거시 vol2/vol3·종합설문은 별도 인라인 JS(이후 확장).
...
## 커피 자아 카드 — 재미·공유 컨텐츠 (fableself 결정 2026-07-16)

Leo 원칙: "참여자도 재밌어야 하고, 공유할 개인 컨텐츠가 형성돼야 의미가 있다." seed를 추출형으로
걷지 말고 '커피 자아 캐릭터'로 되돌려준다. 단 **예측 라벨을 참여자에게 보여주면 랜덤 arm으로도
못 고치는 측정 오염**(priming → 자기실현예언, treatment가 outcome에 직접 개입)이라, 배치를
fableself에 위임 → 결정:

**순서 고정 (측정창 격리 + 피드백 산출물형 리빌)**:
```
퀴즈 → [측정창: 커피 도메인 카드 노출 · 랜덤 arm · 4단계 리액션 수집] → 피드백 제출 lock
     → 자아 카드 리빌 + 공유
```
- 측정창 동안 자아 카드·예측 라벨 **완전 미노출**. lift는 카드 무노출 상태에서 깨끗이 잰다.
- **카드 = 피드백의 산출물**: 측정창 종료 후 그 사람의 리액션을 재료로 생성. 효과 ①측정창 청결
  ②'내 반응의 결과물'이라 개인화·의외성↑→공유가치↑ ③예측 라벨 직접 통보가 아니라 '네 반응을
  종합하니'로 감싸 자기 라벨 고정 완화.
- **lock 필수**: 피드백 제출=commit(수정 불가) 후에만 리빌. 리빌 보고 소급 수정 가능하면 오염 재발.
- **함정 완화**: ①'현재 스냅샷' 프레이밍(고정 정체성 아님) ②예측 축은 2분법 유지, 표현층(캐릭터명·
  한줄)만 피드백 디테일로 변주. ③반전 카드('겉은 블랙, 속은 스위트형')로 의외성 = 공유 훅.

**구현 상태 (2026-07-16)**:
- 로직층 완료 — `engines/coldstart.coffee_reveal(seed, served_item, reaction)`:
  반응이 가리키는 극(reacted)이 주재료, seed의 said와 어긋나면 반전. 반응 미확정 시 seed 폴백,
  둘 다 없으면 커피 새싹. `coffee_persona(seed)`는 said 극 헬퍼로 잔존. 테스트 15개.
- **미배선(다음)**: 프론트 측정창→lock→리빌 흐름. 결과 페이지 커피 카드 리액션을 lock하고
  리빌 카드를 그리는 UX. 게이트(seed_collection) ON일 때만.
...
## 완료 (Leo 승인 후 반영, 점검 Q1·Q3·Q6)

- **키워드 사전 교정 + 축 명칭 정직화** (2026-07-13, Leo 승인): 축을 `black`(진한 블랙형)/
  `sweet`(부드러운 스위트형)으로 리네임(축 a=우유·단맛 유무). 핸드드립·산미·프룻은 축 b
  예약어로 분리(현재 미사용). seed 우도=패밀리당 1회 계상 + 총 LR 캡 3배(곱셈 스태킹 방지).
  pivot을 `predict(..., age_pivot=)` 인자로 변수화. 문헌 계수는 유지(방향성 prior).

## 아직 안 한 것 (의도적 — 별도 작업)

- 추천 아이템의 예측 기반 실제 교체 — 실 lift 검증 후.
- pivot 실제 값 교체(점검 Q1) — 첫 100명 수집 후 유저 평균 연령 주입(인터페이스는 준비됨).
